...
static u32 HelpWord(void) {
    return sSaveFailed                        ? 53
           : sDraft.row < MENU_OPTIONS_FOOTER ? sHelp[sDraft.column * 3 + sDraft.row]
                                              : 33;
}
...
static void ReleaseText(u32 id) {
    ...
    ((void (*)(void*))0x002F6944)(sText[id]);
    for (u32 n = 0; n < 0x50 / 4; ++n)
        sText[id][n] = 0;
    sTextReady[id] = 0;
}
...
static void PrepareText(void) {
    ...
    for (u32 i = 0; i < WORD_COUNT; ++i)
        if (!needed[i])
            ReleaseText(i);
    for (u32 i = 0; i < WORD_COUNT; ++i)
        if (needed[i] && !sTextReady[i]) {
            sBuildingText = sWords[i];
            ((void (*)(void*, u32, int, int, int))0x002F57F0)(sText[i], 0x94f, 0, 0, 0);
            sBuildingText = 0;
            sTextReady[i] = 1;
...
u32 MenuOptions_Lookup(void* context, u32 id, u32* record) {
    if (!sBuildingText)
        return ((u32 (*)(void*, u32, u32*))0x0044D548)(context, id, record);
    u32 length = 0;
    while (sBuildingText[length] && length < 250) {
        sEncoded[length] = sBuildingText[length];
        ++length;
    }
    sEncoded[length++] = 0x7f;
    sEncoded[length++] = 0;
    for (u32 i = 0; i < 24; ++i)
        record[i] = 0;
    record[0] = id;
    for (u32 i = 0; i < 10; ++i) {
        record[4 + i * 2] = (u32)sEncoded;
        record[5 + i * 2] = length;
    }
    return 1;
}
...
void MenuOptions_Begin(void) {
    ...
    for (u32 i = 0; i < 3; ++i)
        sDraft.values[6 + i] = MovementOptions_Get(i);
    sDraft.row = sDraft.column = sRightPage = sSaveFailed = 0;
    sOpen = 1;
...
static void SyncSelection(void) {
    if (sDraft.column && sDraft.row < MENU_OPTIONS_FOOTER)
        sRightPage = sDraft.row / 3;
    ((volatile u32*)0x0050A508)[0x30 / 4] =
        sDraft.row == MENU_OPTIONS_FOOTER ? 3 + sDraft.column : sDraft.row % 3;
}

int MenuOptions_Touch(int x, int y) {
    if (MenuTouch_In(x, y, 132, 132, 43, 16))
        return 0;
    if (MenuTouch_In(x, y, 226, 132, 43, 16))
        return 1;
    if (MenuTouch_In(x, y, 338, 47, 32, 16)) {
        sRightPage ^= 1;
        sDraft.column = 1;
        sDraft.row = sRightPage * 3;
        SyncSelection();
        return -1;
    }
    for (int col = 0; col < 2; ++col) {
        const int left = col ? 207 : 30;
        if (MenuTouch_In(x, y, left, 47, 163, 16))
            MenuOptions_Column(&sDraft, col);
        for (int row = 0; row < 3; ++row) {
            if (!MenuTouch_In(x, y, left + 2, 63 + row * 21, 159, 21))
                continue;
            sDraft.column = col;
            sDraft.row = row + (col ? sRightPage * 3 : 0);
            if (x < left + 27)
                MenuOptions_Change(&sDraft, -1);
            else if (x >= left + 135)
                MenuOptions_Change(&sDraft, 1);
        }
    }
...
void MenuOptions_ExitNative(int accept) {
    if (accept) {
        ...
        if (!MenuPreferences_Save(sDraft.values)) {
            sSaveFailed = 1;
            return;
        }
        sSaveFailed = 0;
        ...
        for (u32 i = 0; i < 3; ++i)
            native[i] = sDraft.values[i];
        volatile u8* settings = (volatile u8*)0x00587958;
        settings[0x2d] = sDraft.values[0];
        settings[0x13d8] = sDraft.values[1];
        ...
        for (u32 i = 0; i < 3; ++i)
            MovementOptions_Set(i, sDraft.values[6 + i]);
    }
    volatile u32* save = (volatile u32*)0x0050A508;
    save[0x30 / 4] = accept ? 4 : 3;
    save[0x3c / 4] = 2;
    sNativeExit = 1;
...
static void DrawPanels(void) {
    ...
    if (!nodes[active]) {
        ...
        for (u32 q = 0; q < 78; ++q) {
            ...
            /* A quiet inset for contextual help, clear of native buttons. */
            if (q >= 16 && q < 21) {
                l = 80;
                t = 158;
                w = 240;
            ...
            for (u32 v = 0; v < 4; ++v) {
                ...
                uvs[active][q * 8 + v * 2 + 1] =
                    1 - (part == 0 ? 8.5f + 231 * (v & 1) : 80.5f) / 256;
                const u8 border = part >= 1 && part <= 4;
                float shade = part == 0 ? 0.30f : part == 5 ? 0.24f : 0.45f;
                if (border)
                    shade = selected ? 1.0f : 0.70f;
                if (part == 3 || part == 4)
                    shade *= 0.65f;
                if (q >= 16)
                    shade = q == 16 ? 0.18f : 0.48f;
        ...
        for (u32 i = 0; i < 0x118 / 4; ++i)
            descriptor[i] = ((const u32*)0x004D54D0)[i];
        descriptor[0] = (u32)positions[active];
        descriptor[1] = (u32)uvs[active];
        descriptor[2] = (u32)colors[active];
        descriptor[4] = (u32)indices;
        ((void* (*)(void*, void*))0x00348F34)(resources[active], descriptor);
        ...
        nodes[active] = ((void* (*)(void*, void*, void*, u32))0x0034897C)(*(void**)0x005C0A34,
                                                                          resources[active], 0, 0);
        if (nodes[active])
            *(u32*)((u8*)nodes[active] + 0x178) |= 2;
    }
    if (nodes[active])
...
void MenuOptions_Draw(void) {
    ...
    for (u32 col = 0; col < 2; ++col)
        for (u32 row = 0; row < 3; ++row) {
            ...
            Text(35, x, y + 10, 0.62f, focus);
            Text(36, x + 132, y + 10, 0.62f, focus);
        }
