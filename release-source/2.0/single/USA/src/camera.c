...
static u8 Camera_StickAvailable(void) {
#if UNIFIED_MENU_PROTOTYPE
    return !InputRemap_IsGearPageActive();
#else
    return 1;
#endif
}
...
static u8 sDistanceOption = 2;

uint8_t Camera_GetDistanceOption(void) { return sDistanceOption; }
void Camera_SetDistanceOption(uint8_t option) { sDistanceOption=option>6 ? 6 : option; }
...
u8 Camera_FreeCamEnabled(Camera* camera) {
    ...
    if (Camera_StickAvailable() && rInputCtx.cStick.dx * rInputCtx.cStick.dx + rInputCtx.cStick.dy * rInputCtx.cStick.dy > 900) {
        freeCamEnabled = 1;
    }
...
void Camera_FreeCamUpdate(Vec3s* out, Camera* camera) {
    ...
    if (camera->player) {
        ...
        const float distanceScale=0.75f+0.125f*sDistanceOption;
        dist = lerpf(dist, (((gSaveContext.linkAge) ? 150 : 200) - 50 * sins(pitch))*distanceScale, 0.5);
