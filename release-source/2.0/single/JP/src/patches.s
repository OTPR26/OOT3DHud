...
.if (_JP_==1) && (_JP_SINGLE_SCREEN_PROBE_==1) && (UNIFIED_MENU_PROTOTYPE==0)
...
.if (_USA_==1) || (_EUR_==1) || (_JP_==1)
.section .patch_UnifiedMenuRepeat
    b UnifiedMenu_GetNativeMenuRepeat
.section .patch_UnifiedMenuButtons
    b UnifiedMenu_GetNativeMenuButtons
.section .patch_UnifiedMenuCursor
    b hook_UnifiedMenuCursor
.endif

@ EUR uses the same input-hook sites; regional continuations are kept in hooks.s.
.if (_USA_==1) || ((_EUR_==1) && (_EUR_SINGLE_SCREEN_PROBE_==1)) || ((_JP_==1) && (UNIFIED_MENU_PROTOTYPE==1))
...
.if UNIFIED_MENU_PROTOTYPE==1
    bl SingleScreen_DrawDialogueBoards
.else
...
.if (_USA_==1) || (_EUR_==1) || (_JP_==1)
.if UNIFIED_MENU_PROTOTYPE==1
.section .patch_UnifiedPauseDim
    bl SingleScreen_DrawPauseDim
.section .patch_DialogueTextScale
    bl SingleScreen_DrawDialogueText
.section .patch_ItemsShelfColor
    bl UnifiedMenu_InitShelfColors
.section .patch_GearShelfColor
    bl UnifiedMenu_InitShelfColors
.section .patch_GearPreviewCamera
    bl GearPreview_DrawCamera
.section .patch_MenuOptionsTextLookup
    bl MenuOptions_Lookup
.section .patch_MovementSpeed
    bl hook_MovementSpeed
.section .patch_MovementAnimation
    bl hook_MovementAnimation
.section .patch_RollRecovery
    bl hook_RollRecovery
.section .patch_SingleScreenBeforeTopPresentation
.endif
.endif
...
// The USA/EUR HD QBF uses 32x32 cells and needs a 512x512 atlas plus
// half-height layout advances. Japan retains its native 16x16 std16.qbf;
// applying these patches there halves Japanese line spacing and overlaps text.
.if (_USA_==1) || (_EUR_==1)
