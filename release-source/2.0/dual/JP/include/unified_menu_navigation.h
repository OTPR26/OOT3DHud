...
static inline int MenuMap_DungeonCanMoveDown(int selected,
                                            const volatile unsigned* floors) {
    if (selected < 0 || selected > 7) return 0;
    if (selected < 4) return floors[selected + 1] != 0;
...
static inline int MenuMap_WorldCanMoveDown(unsigned selected,
    ...
    if (selected >= 12) return 0;
    unsigned next = neighbours[selected * 8 + 4];
    if (next >= 12) return 0;
    if (available[next]) return 1;
