...
.global hook_UnifiedMenuCursor
hook_UnifiedMenuCursor:
    push {r0-r3, r12, lr}
    bl UnifiedMenu_GetFooterFocus
    cmp r0, #0
    pop {r0-r3, r12, lr}
    beq 1f
    // Type 0 is the gold selection cursor; preserve equipped outlines.
    ldr r12, [r0, #0x24]
    cmp r12, #0
    bxeq lr
1:
    // Displaced native load; resume the cached cursor node draw.
    ldr r0, [r0, #4]
.if _JP_==1
    b 0x2FB450
.else
...
.text
.global hook_FilterTouch
hook_FilterTouch:
    push {r0-r12, lr}
    mov r0,r4
    add r1,sp,#64
    bl InputRemap_FilterTouch
    pop {r0-r12, lr}
    b 0x2F9F1C
