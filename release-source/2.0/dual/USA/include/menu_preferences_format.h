...
static inline int PrefValuesValid(const uint8_t* v) {
    return v[0] < 2 && v[1] < 2 && v[2] < 2 && v[3] < 4 && v[4] < 7 && v[5] < 4 && v[6] < 3 &&
           v[7] < 3 && v[8] < 3;
}
static inline void PrefEncode(uint8_t* p, unsigned slot, uint32_t identity, uint32_t generation,
                              const uint8_t* v) {
    memset(p, 0, PREF_SIZE);
    memcpy(p, "ORPF", 4);
    p[4] = 2;
    p[5] = slot;
    memcpy(p + 6, v, 6);
    PrefPut32(p + 12, identity);
    PrefPut32(p + 16, generation);
    memcpy(p + 20, v + 6, 3);
