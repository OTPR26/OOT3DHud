...
.section .patch_JapanMap
    b hook_JapanMap
.endif
.if (_KOR_==1) && (_KOR_SINGLE_SCREEN_PROBE_==1)
.section .patch_KoreaFileBefore
    bl hook_KoreaFileBefore
.section .patch_KoreaFileAfter
    bl hook_KoreaFileAfter
.section .patch_KoreaFileText
    blne SingleScreen_DrawFileText
.section .patch_KoreaFileBackdrop
    bl hook_KoreaFileBackdrop
.section .patch_KoreaFileHeading
    bl SingleScreen_UpdateFileBoard
.section .patch_KoreaProbeTopPass
    bl KoreaProbe_AfterTopPass
.section .patch_KoreaProbePresentation
    bl hook_KoreaProbePresentation
.section .patch_KoreaProbeOverlay
    bl KoreaProbe_Overlay
.section .patch_KoreaProbeTitle
    bl KoreaProbe_Title
.section .patch_KoreaProbeLeather
    bl KoreaProbe_Backdrop
.section .patch_KoreaProbeItems
    b hook_KoreaProbeItems
.section .patch_KoreaProbeGear
    b hook_KoreaProbeGear
.section .patch_KoreaProbeMap
    b hook_KoreaProbeMap
.section .patch_KoreaProbeCallback
    blne KoreaProbe_SecondaryCallback
.endif
...
.if ((_JP_==1) && (_JP_SINGLE_SCREEN_PROBE_==1)) || ((_TWN_==1) && (_TWN_SINGLE_SCREEN_PROBE_==1)) || ((_KOR_==1) && (_KOR_SINGLE_SCREEN_PROBE_==1))
