...
.global hook_KoreaFileBefore
hook_KoreaFileBefore:
    push {r0-r12, lr}
    vpush {d0-d7}
    bl SingleScreen_BeforeFileScene
    vpop {d0-d7}
    pop {r0-r12, lr}
    b 0x0010C23C
.global hook_KoreaFileBackdrop
hook_KoreaFileBackdrop:
    push {r0-r12, lr}
    vpush {d0-d7}
    bl SingleScreen_BeforeFileBackdrop
    vpop {d0-d7}
...
hook_KoreaFileAfter:
    ...
    pop {r0-r12, lr}
    pop {r4, pc}
.macro korea_page_hook name, page, target, gear=0
.global \name
\name:
    push {r0-r12, lr}
    vpush {d0-d7}
...
.endif
    b \target
.endm
korea_page_hook hook_KoreaProbeItems, 1, 0x00315E70
korea_page_hook hook_KoreaProbeGear, 2, 0x0030D4E4, 1
korea_page_hook hook_KoreaProbeMap, 3, 0x00315B30

.global hook_KoreaProbePresentation
hook_KoreaProbePresentation:
    push {r0-r12, lr}
    vpush {d0-d7}
    bl KoreaProbe_PositionPresentation
    vpop {d0-d7}
    pop {r0-r12, lr}
    b 0x001154B0
.endif
