...
void SingleScreen_DrawDialogueBoards(void* pass) {
    ...
    ((FinishTopPassFn)ADDR(0x004228B8))(pass);
...
void SingleScreen_DrawDialogueText(void* message) {
    ...
    const u8 compact = IsGameplayDialogueActive();
    if (compact) ApplyDialogueViewport();
    ((void (*)(void*))ADDR(0x0042CBA8))(message);
    if (compact) SetViewport(0, 0, 480, 400);
}
#endif
...
#if UNIFIED_MENU_PROTOTYPE && defined(Version_JP)
static void ScaleFileQuads(float* p, const float* t, u32 first, u32 count) {
...
void SingleScreen_UpdateFileBoard(void* board) {
    ...
    if (offsets) {
        for (u32 action = 0; action < 3; ++action) {
            ...
            /* The native pressed-state frame is a separate three-slice
             * layer (175..183). It becomes visible on confirmation, so
             * resizing only the face leaves a full-size dark outline.
             * Use the face's pivot but each frame slice's own translation. */
            const u32 pressedFirst = 175 + action*3;
            for (u32 q = pressedFirst; q < pressedFirst+3; ++q) {
                float* p = positions + q*12;
                const float frameCx = actionX[action] + offsets[q*2];
                const float frameCy = actionY[action] + offsets[q*2+1];
                for (u32 v = 0; v < 4; ++v) {
                    p[v*3] = frameCx + (p[v*3] - frameCx)*actionScaleX[action];
                    p[v*3+1] = frameCy + (p[v*3+1] - frameCy)*actionScaleY[action];
                }
            }
#endif
        }
    ...
    if (fileState >= 11 && fileState <= 42 && offsets) {
        for (u32 action = 0; action < 4; ++action)
            ScaleFileQuads(positions, offsets, 134 + action * 7, 7);
        u32* cursor = *(u32**)0x00504FD4;
        u32* cursorBoard = cursor ? (u32*)cursor[2] : 0;
        if (cursorBoard && cursor[9] == 0 && cursorBoard[0] == 8) {
            float* p = (float*)cursorBoard[3];
            float* t = (float*)cursorBoard[7];
            if (p && t) ScaleFileQuads(p, t, 0, 8);
        }
    }
...
void SingleScreen_DrawPauseDim(void* context, u32 kind, const float* color, u32 flags) {
    ...
    ((void (*)(void*, u32, const float*, u32))ADDR(0x003339E8))
...
void SingleScreen_AfterTopPass(void* pass) {
    ...
    if (InputRemap_IsGameOverUiOpen()) {
        ...
        if (deathMenu != 0 && gGlobalContext != 0) {
            ((void (*)(void*, GlobalContext*))ADDR(0x0041C500))(
    ...
    if (InputRemap_IsItemsMenuOpen()) {
        ...
        const u8 dungeonMap = !InputRemap_IsItemsPageActive() &&
            !InputRemap_IsGearPageActive() && ((int (*)(void))ADDR(0x002F1268))();
...
#if defined(Version_JP) || defined(Version_TWN)
        if (replayBack) SetScissor(1, 56, 335, 110, 399);
        else SetScissor(0, 0, 0, 479, 399);
#else
        // Include the filtered border around the original Back button.
        if (replayBack) SetScissor(1, 54, 346, 109, 399);
        else SetScissor(0, 0, 0, 479, 399);
#endif
        DrawFileSelectionUi();
        if (replayBack) {
