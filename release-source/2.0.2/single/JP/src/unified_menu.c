...
static void TintShelf(void* board, u32 expected, u32 first, u32 count,
                      float green, float blue) {
    if (!board || *(u32*)board != expected) return;
...
void UnifiedMenu_InitShelfColors(void* board, const float* alpha, int count, int first) {
    ((void(*)(void*,const float*,int,int))ADDR(0x002FCDEC))(board,alpha,count,first);
    /* At construction tint only this new board, not other menu lifetimes. */
    TintShelf(board,37,0,2,1.0f,1.0f);
    /* The mirrored shelf halves meet at atlas x124, exactly beside the pale
     * stone artwork. Linear filtering there creates a bright center seam.
     * Constructor UVs already exist here, before the GPU resource is built.
     * Inset only the shared edge to the last blue texel's center. */
    if (board && *(u32*)board == 37) {
        float* uv=*(float**)((u8*)board+0x14);
        if (uv) {
            uv[0*8+1*2]=uv[0*8+3*2]=123.5f/256.0f;
            uv[1*8+0*2]=uv[1*8+2*2]=123.5f/256.0f;
        }
    }
}
...
void UnifiedMenu_DrawBackdrop(void) {
    if (!InputRemap_IsItemsMenuOpen() ||
        ...
        UnifiedMenu_IsOpening() || UnifiedMenu_IsClosing()) return;
    ...
    if (!node) {
        ...
        for (u32 q=0;q<78;++q) {
            ...
            for (u32 i=0;i<6;++i) indices[q*6+i]=q*4+offsets[i];
        }
        u32 descriptor[0x120/4];
        const u32* nativeTemplate=(const u32*)0x004D54D0;
        for (u32 i=0;i<0x118/4;++i) descriptor[i]=nativeTemplate[i];
        descriptor[0]=(u32)positions; descriptor[1]=(u32)uvs;
        descriptor[2]=(u32)colors; descriptor[4]=(u32)indices;
        ((void*(*)(void*,void*))ADDR(0x00348F34))(resource,descriptor);
        void* texture=((void*(*)(u32))ADDR(0x002E11D0))(6);
        ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
            (resource,0,texture,0x2601,0x2601,0x812f,0x812f);
        node=((void*(*)(void*,void*,void*,u32))ADDR(0x0034897C))
            (*(void**)0x005C0A34,resource,0,0);
        if (node) *(u32*)((u8*)node+0x178)|=2;
    }
    if (node) {
...
void UnifiedMenu_DrawDollBackdrop(void) {
    if (!InputRemap_IsGearPageActive() || UnifiedMenu_IsOpening() ||
        UnifiedMenu_IsClosing() || UnifiedMenu_GetDialogPage()) return;
    static float positions[78*12] __attribute__((aligned(16)));
    static float uvs[78*8] __attribute__((aligned(16)));
    static float colors[78*16] __attribute__((aligned(16)));
    static u16 indices[78*6];
    ...
    if (!node) {
        ...
        for (u32 q=0;q<78;++q) {
            for (u32 v=0;v<4;++v) {
                ...
                colors[q*16+v*4+3]=(q<9 && x>0 && x<3 && y>0 && y<3) ? 0.18f : 0;
            }
            for (u32 i=0;i<6;++i) indices[q*6+i]=q*4+offsets[i];
        }
        u32 descriptor[0x120/4];
        ...
        descriptor[2]=(u32)colors; descriptor[4]=(u32)indices;
        ((void*(*)(void*,void*))ADDR(0x00348F34))(resource,descriptor);
        void* texture=((void*(*)(u32))ADDR(0x002E11D0))(3);
        ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
            (resource,0,texture,0x2601,0x2601,0x812f,0x812f);
        node=((void*(*)(void*,void*,void*,u32))ADDR(0x0034897C))
            (*(void**)0x005C0A34,resource,0,0);
    ...
    if (!sFocusNodes[index]) {
        ...
        for (u32 q=0;q<78;++q) {
            for (u32 v=0;v<4;++v) {
                ...
                sFocusUvs[q*8+v*2+1]=1-((q&2 ? 16 : 0)+(v>=2 ? 15.5f : 0.5f))/128;
            }
        }
        u32 descriptor[0x120/4];
        const u32* nativeTemplate=(const u32*)0x004D54D0;
        ...
        ((void*(*)(void*,void*))ADDR(0x00348F34))(resource,descriptor);
        void* texture=((void*(*)(u32))ADDR(0x002E11D0))(13);
        ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
            (resource,0,texture,0x2601,0x2601,0x812f,0x812f);
        sFocusNodes[index]=((void*(*)(void*,void*,void*,u32))ADDR(0x0034897C))
            (*(void**)0x005C0A34,resource,0,0);
        if (sFocusNodes[index]) *(u32*)((u8*)sFocusNodes[index]+0x178)|=2;
    }
    void* node=sFocusNodes[index];
    if (node) ((void(*)(void*))(*(u32*)(*(u32*)node+12)))(node);
}
    ...
    if (!sFooterNodes[focus]) {
        ...
        for (u32 q=0;q<78;++q) {
            ...
            for (u32 v=0;v<4;++v) {
                ...
                sFooterPositions[q*12+v*3] = save ?
                    (45+(px-162)*70/124)*0.8f : options ?
                    (285+(px-110)*70/100)*0.8f : -100;
                sFooterPositions[q*12+v*3+1] = save ?
                    209+(py-94)*20/52 : options ? 209+(py-202)*20/36 : -100;
                sFooterPositions[q*12+v*3+2]=0;
                for (u32 c=0;c<4;++c)
                    sFooterColors[q*16+v*4+c] = 1.0f;
            }
            for (u32 i=0;i<8;++i) sFooterUvs[q*8+i]=uv[q*8+i];
            const u16 offsets[6]={0,2,1,1,2,3};
            for (u32 i=0;i<6;++i) sFooterIndices[q*6+i]=q*4+offsets[i];
        }
        u32 descriptor[0x120/4];
        const u32* nativeTemplate=(const u32*)0x004D54D0;
        for (u32 i=0;i<0x118/4;++i) descriptor[i]=nativeTemplate[i];
        ...
        ((void*(*)(void*,void*))ADDR(0x00348F34))(resource,descriptor);
        void* texture=((void*(*)(u32))ADDR(0x002E11D0))(12);
        ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
            (resource,0,texture,0x2601,0x2601,0x812f,0x812f);
        sFooterNodes[focus]=((void*(*)(void*,void*,void*,u32))ADDR(0x0034897C))
            (*(void**)0x005C0A34,resource,0,0);
        if (sFooterNodes[focus]) *(u32*)((u8*)sFooterNodes[focus]+0x178)|=2;
    }
...
static float* GetPositions(void) {
    ...
    if (!board || *(u32*)board != 37) return 0;
...
static void* CreateHeader(u32 active) {
    ...
    for (u32 i=0;i<0x118/4;++i) sHeaderDescriptor[i]=nativeTemplate[i];
    sHeaderDescriptor[0]=(u32)sHeaderPositions;
    const float* nativeUvs=*(float**)(board+0x14);
    for (u32 i=0;i<37*8;++i) sHeaderUvs[i]=nativeUvs[i];
#if defined(Version_EUR)
    /* EUR's replacement Items sprite has its own lower stone frame. JP uses
     * its original Japanese tab and the separate native frame instead. */
    for (u32 v=2;v<4;++v) sHeaderUvs[11*8+v*2+1]=1.0f-250.0f/256.0f;
#endif
    ...
    for (u32 v=0;v<2;++v) sHeaderUvs[10*8+v*2+1]-=0.5f/256.0f;
    ...
    for (u32 r=0;r<4;++r) {
        const u32 q=rails[r];
        for (u32 v=0;v<4;++v) {
            sHeaderUvs[q*8+v*2]=(r<2 ? (v&1 ? 178.5f : 150.5f) : (v>=2 ? 178.5f : 150.5f))/256.0f;
            sHeaderUvs[q*8+v*2+1]=1.0f-(r<2 ? (v<2 ? 140.5f : 144.5f) : (v&1 ? 144.5f : 140.5f))/256.0f;
        }
    }
    ...
    for (u32 i=0;i<37*16;++i) sHeaderColors[i]=1.0f;
    ...
    for (u32 q=0;q<37;++q) {
        const u16 offsets[6]={0,2,1,1,2,3};
        for (u32 i=0;i<6;++i) sHeaderIndices[q*6+i]=q*4+offsets[i];
    }
    ((void*(*)(void*,void*))ADDR(0x00348F34))(resource,sHeaderDescriptor);
    void* texture=((void*(*)(u32))ADDR(0x002E11D0))(3);
    ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
        (resource,0,texture,0x2601,0x2601,0x812f,0x812f);
    void* node=((void*(*)(void*,void*,void*,u32))ADDR(0x0034897C))
        (*(void**)0x005C0A34,resource,0,0);
    if (node) *(u32*)((u8*)node+0x178)|=2;
    return node;
}
...
static void DrawShelfFrame(void) {
    if (!InputRemap_IsItemsMenuOpen() || UnifiedMenu_GetDialogPage() ||
        UnifiedMenu_IsOpening() || UnifiedMenu_IsClosing()) return;
    const u32 gear=InputRemap_IsGearPageActive() ? 1 : 0;
    if (!gear && !InputRemap_IsItemsPageActive()) return;
    ...
    if (!nodes[gear]) {
        ...
        const float rect[6][4]={
            {126,48,4,132},{20,148.5f,216,4},
        ...
        for (u32 q=0;q<37;++q) {
            ...
            for (u32 v=0;v<4;++v) {
                    ...
                    if (gear) {
                        if (q<2) {
                            ...
                            if (side==0 || side==3) {
                                ...
                                x=(v&1) ? r : l;
                                if (inner) x+=(v&1) ? -width : width;
                                y=side==0 ? t+(v>=2 ? width : 0) : b-(v<2 ? width : 0);
                            } else {
                                const u8 inner=side==1 ? (v&1) : !(v&1);
                                y=v>=2 ? b : t;
                                if (inner) y+=v>=2 ? -width : width;
                                x=side==1 ? l+((v&1) ? width : 0) : r-(!(v&1) ? width : 0);
                            }
                        }
                    } else {
                ...
                uv[gear][q*8+v*2]=(gear ? (v&1 ? 100.5f : 80.5f)/512 :
                                                   (v&1 ? 90.5f : 70.5f)/256);
                uv[gear][q*8+v*2+1]=1-(gear ? (q>=2 ? 0.5f :
                                              (v>=2 ? 50.5f : 30.5f)) : 3.5f)/256;
                if (gear) {
                    ...
                    const float u=q==0 ? ((v&1) ? 88.5f : 82.5f) :
                                  q==1 ? ((v&1) ? 154.5f : 4.5f) : 84.5f;
                    const float ty=q==0 ? (v>=2 ? 140.5f : 4.5f) :
                                   q==1 ? (v>=2 ? 74.5f : 70.5f) : 0.5f;
                    uv[gear][q*8+v*2]=u/512;
                    uv[gear][q*8+v*2+1]=1-ty/256;
                }
                float shade=1;
                if (!gear) shade=v<2 ? 1.0f : 0.62f;
                else if (q==2) shade=v<2 ? 1.0f : 0.65f;
                else if (q==3) shade=(v&1) ? 0.60f : 0.90f;
                else if (q==4) shade=(v&1) ? 0.35f : 0.65f;
                else if (q==5) shade=v<2 ? 0.70f : 0.35f;
                else if (q>=6) shade=0.28f;
                for (u32 c=0;c<3;++c) color[gear][q*16+v*4+c]=shade;
        ...
        const u32* nativeTemplate=(const u32*)0x004D4984;
        for (u32 i=0;i<0x118/4;++i) descriptor[i]=nativeTemplate[i];
        descriptor[0]=(u32)pos[gear]; descriptor[1]=(u32)uv[gear];
        descriptor[2]=(u32)color[gear]; descriptor[4]=(u32)indices;
        ((void*(*)(void*,void*))ADDR(0x00348F34))(resources[gear],descriptor);
        /* USA texture-name table 4D4010: Items slot3, Gear slot4. */
        void* texture=((void*(*)(u32))ADDR(0x002E11D0))(gear ? 4 : 3);
        ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
            (resources[gear],0,texture,0x2601,0x2601,0x812f,0x812f);
        nodes[gear]=((void*(*)(void*,void*,void*,u32))ADDR(0x0034897C))
            (*(void**)0x005C0A34,resources[gear],0,0);
        if (nodes[gear]) *(u32*)((u8*)nodes[gear]+0x178)|=2;
    }
...
static void DrawMapFrame(void) {
    if (!InputRemap_IsItemsMenuOpen() || InputRemap_IsItemsPageActive() ||
        InputRemap_IsGearPageActive() || UnifiedMenu_GetDialogPage() ||
        UnifiedMenu_IsOpening() || UnifiedMenu_IsClosing()) return;
    static float positions[2][37*12], uvs[2][37*8], colors[2][37*16];
    static u16 indices[37*6];
    static u32 resources[2][0x1b8/4] __attribute__((aligned(16)));
    static void* nodes[2];
    const u32 dungeon = ((int (*)(void))ADDR(0x002F1268))() ? 1 : 0;
    ...
    if (!node) {
        ...
        const float rect[8][4]={
            {9,41,222,2},{9,43,2,140},{229,43,2,140},{9,183,222,2},
            {11,43,218,1},{11,44,1,138},{228,44,1,138},{11,182,218,1}
        };
        const u16 order[6]={0,2,1,1,2,3};
        for (u32 q=0;q<37;++q) {
            for (u32 v=0;v<4;++v) {
                ...
                if (q<8 && dungeon) {
                    ...
                    if (pos[q*12+v*3+1]>=182) pos[q*12+v*3+1]+=4;
                }
                if (q<8 && !dungeon)
                ...
                const float shade=q>=4 ? 0.08f :
                    q==0 ? (v<2 ? 0.95f : 0.64f) :
                    q==1 ? (v&1 ? 0.64f : 0.88f) :
                    q==2 ? (v&1 ? 0.42f : 0.70f) : (v<2 ? 0.68f : 0.42f);
                for (u32 c=0;c<3;++c) color[q*16+v*4+c]=shade;
        ...
        const u32* nativeTemplate=(const u32*)0x004D4984;
        for (u32 i=0;i<0x118/4;++i) descriptor[i]=nativeTemplate[i];
        descriptor[0]=(u32)pos; descriptor[1]=(u32)uv;
        descriptor[2]=(u32)color; descriptor[4]=(u32)indices;
        ((void*(*)(void*,void*))ADDR(0x00348F34))(resource,descriptor);
        void* texture=((void*(*)(u32))ADDR(0x002E11D0))(3);
        ((void(*)(void*,u32,void*,u32,u32,u32,u32))ADDR(0x00348A64))
            (resource,0,texture,0x2601,0x2601,0x812f,0x812f);
...
void UnifiedMenu_PrepareDungeonMaterials(void) {
    if (!InputRemap_IsItemsMenuOpen() || InputRemap_IsItemsPageActive() ||
        InputRemap_IsGearPageActive() || !((int (*)(void))ADDR(0x002F1268))()) return;
...
void UnifiedMenu_DrawDungeonFooterMask(void) {
    if (!InputRemap_IsItemsMenuOpen() || InputRemap_IsItemsPageActive() ||
        ...
        !((int (*)(void))ADDR(0x002F1268))()) return;
    ...
    if (!node) {
        ...
        for (u32 q=0;q<37;++q) {
            for (u32 v=0;v<4;++v) {
                ...
                pos[q*12+v*3+2]=0;
                uv[q*8+v*2]=(q==0 ? (v&1 ? 63.5f : 0.5f) :
                    (v&1 ? 463.5f : 256.5f))/512;
                uv[q*8+v*2+1]=1-(q==0 ? (v>=2 ? 239.5f : 198.5f) :
                    (v>=2 ? 131.5f : 168.5f))/256;
                color[q*16+v*4]=1;
                color[q*16+v*4+1]=1;
                color[q*16+v*4+2]=1;
...
static void PlaceQuad(float* source, u32 q, float sx, float sy,
                      float sw, float sh, float x, float y, float w, float h) {
    for (u32 v = 0; v < 4; ++v) {
        u32 i = q * 12 + v * 3;
...
void UnifiedMenu_DrawHeader(void) {
    ...
    if (InputRemap_IsSaveMenuOpen() || UnifiedMenu_IsOptionsDialog()) return;
    if (!InputRemap_IsItemsMenuOpen() && !dialogPage) return;
    ...
    const u32 active = dialogPage ? dialogPage-1 : InputRemap_IsItemsPageActive() ? 0 :
                       InputRemap_IsGearPageActive() ? 1 : 2;
    ...
    for (u32 t = 0; t < 3; ++t) {
        ...
        /* JP's original localized Items tab uses the native three-slice
         * frame; EUR's replacement sprite has its own. */
        for (u32 q = frames[t];
#if defined(Version_EUR)
             t != 0 &&
#endif
    ...
    if (!sHeaderNodes[active]) sHeaderNodes[active]=CreateHeader(active);
    void* node=sHeaderNodes[active];
    if (!node) return;
    ((void (*)(unsigned,int,unsigned,unsigned))ADDR(0x002FEABC))(0,0,480,400);
    void (*draw)(void*) = (void (*)(void*))(*(u32*)(*(u32*)node+12));
    draw(node);
    if (!dialogPage) DrawFooter();
    DrawShelfFrame();
    DrawMapFrame();
