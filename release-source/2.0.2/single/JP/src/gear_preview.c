...
static void GearPreview_SpreadJoint(float* local, const float* world, float degrees) {
    ...
    for (u32 row=0;row<3;++row) {
        ...
        for (u32 col=0;col<3;++col)
            local[row*4+col]=a*rotation[col]+b*rotation[3+col]+d*rotation[6+col];
    }
}
...
static void GearPreview_AnimateJoints(u32* skeleton, u32 bones, float* matrix,
    ...
    if (bones != 25) {
        if (!reported) { CitraPrint("GEAR-IDLE unsupported bones=%u\n",bones); reported=1; }
        return;
    }
    const u8* descriptors=(const u8*)((const u32*)skeleton[1])[1];
    if (!descriptors || !skeleton[5]) return;
    for (u32 i=0;i<25;++i)
        if (*(const s16*)(descriptors+i*40+2) != parents[i]) {
            if (!reported) { CitraPrint("GEAR-IDLE hierarchy mismatch bone=%u\n",i); reported=1; }
            return;
        }
    if (!reported) { CitraPrint("GEAR-IDLE joint animation active bones=%u\n",bones); reported=1; }
    ...
    for (u32 row=0;row<3;++row)
        for (u32 col=0;col<3;++col) head[row*4+col]=neutralHead[row*4+col];
    ...
    ((void (*)(void*,const float*))ADDR(0x0030478C))(skeleton,matrix);
    ...
    for (u32 row=0;row<3;++row)
        matrix[row*4+3] += feet[row] -
            (world[5*12+row*4+3]+world[8*12+row*4+3])*0.5f;
    ((void (*)(void*,const float*))ADDR(0x0030478C))(skeleton,matrix);
}
...
static void GearPreview_TestOwnership(const Player* player,
                                      const SkeletonAnimationModel* live) {
    static u8 attempted;
    if (attempted) return;
    attempted = 1;
    u32* factory = *(u32**)0x005BE734;
    if (!factory || !factory[0]) return;
    ...
    CitraPrint("GEAR-LINK factory=%08x create=%08x resource=%08x\n",
        (u32)factory, create, resource);
    if (create != 0x003FF53C || !resource ||
        ((const u32*)live->vtbl)[1] != 0x003FEFC0) {
        CitraPrint("GEAR-LINK ownership test skipped: identity mismatch\n");
        return;
    }
    const u32* original = (const u32*)live;
    u32 before[8];
    for (u32 i = 0; i < 8; ++i) before[i] = original[i];
    ...
    for (u32 i = 1; i < 8; ++i) {
        if (!owned[i]) continue;
        for (u32 j = 1; j < 8; ++j)
            if (owned[i] == before[j]) independent = 0;
    }
    ...
    if (!independent || !preview->vtbl ||
        ((const u32*)preview->vtbl)[1] != 0x003FEFC0) {
    ...
    u8 unchanged = 1;
    for (u32 i = 0; i < 8; ++i)
        if (original[i] != before[i]) unchanged = 0;
...
void GearPreview_DrawCamera(void* queue) {
#if UNIFIED_MENU_PROTOTYPE && (defined(Version_USA) || defined(Version_EUR) || defined(Version_JP))
    void (*nativeDraw)(void*) = (void (*)(void*))ADDR(0x004224E8);
    const u8 active = gSaveContext.gameMode == 0 && InputRemap_IsGearPageActive() &&
        !UnifiedMenu_IsOpening() && !UnifiedMenu_IsClosing() && !UnifiedMenu_GetDialogPage();
    if (!queue || !active || !sDrawReady || !sPreview || sRetired) { nativeDraw(queue); return; }
    ...
    if (savedCount > 32) { nativeDraw(queue); return; }
    ...
    if (meshCount && meshCount <= 256 && meshCount == *(const u32*)(liveRaw + 0x68)) {
        ...
        if (dst && src && dst != src)
            for (u32 i=0; i<meshCount; ++i) dst[i] = src[i];
    }
    ...
    if (state && liveState && state != liveState) {
        for (u32 i=4; i<10; ++i) ((u8*)state)[i] = ((const u8*)liveState)[i];
        for (u32 i=0x0c/4; i<0x6c/4; ++i) state[i] = liveState[i];
    }
    ...
    ((void (*)(void*,const float*))ADDR(0x003721E0))(sPreview, matrix);
    ((u8*)sPreview)[0xac] = 1;
    ((u8*)sPreview)[0xad] = 1;
    ...
    if (bones && bones <= 128 && skeleton[1] == liveSkeleton[1] &&
        skeleton[4] && liveSkeleton[4] &&
        skeleton[4] != liveSkeleton[4] && skeleton[12] == 0) {
        float neutralHead[12]={0};
        if (bones>11)
            for (u32 i=0;i<12;++i) neutralHead[i]=((const float*)skeleton[4])[11*12+i];
        ((void (*)(void*,const float*))ADDR(0x0030478C))(skeleton, matrix);
    ...
    if (mapper && liveMapper && mapper[0] == liveMapper[0] && mapper[1] != liveMapper[1]) {
        ...
        if (materialCount <= 128 && mapper[1] && liveMapper[1]) {
            ...
            for (u32 m=0; m<materialCount; ++m) {
                ...
                for (u32 i=0;i<0x118;++i) dst[i]=src[i];
                const u32* table=(const u32*)(src+0x118);
                if (table[0]<=256 && (!table[0] || (table[1] && table[2]))) {
                    for (u32 i=0;i<3;++i) ((u32*)(dst+0x118))[i]=table[i];
                } else {
                    dst[7]=dst[8]=dst[9]=0;
                }
            }
    ...
    *count = 1;
    ...
    ((void (*)(unsigned,int,unsigned,unsigned))ADDR(0x002FEABC))
        (48,-30-MENU_GEAR_PORTRAIT_SHIFT,336,252);
    nativeDraw(queue);
    for (u32 m=0;m<syncedMaterials;++m)
        for (u32 i=0;i<3;++i)
            ((u32*)((u8*)mapper[1]+m*0x124+0x118))[i]=savedTextureTables[m][i];
    ((void (*)(unsigned,int,unsigned,unsigned))ADDR(0x002FEABC))(88,6,240,180);
    *count = savedCount;
    record[0] = saved;
    record[1] = savedKind;
    static u8 reported;
    if (!reported) { CitraPrint("GEAR-LINK independent camera draw returned\n"); reported = 1; }
#else
    (void)queue;
#endif
}
    ...
    if (cameraCount <= 32) {
        for (u32 i = 0; i < cameraCount; ++i) {
            ...
            CitraPrint("GEAR-LINK camera-index=%u model=%08x kind=%u\n",
                i, record[0], record[1] & 255);
            if (!model || (record[1] & 255) != 0) continue;
            ...
            for (u32 row = 0; row < 3; ++row) {
                ...
                CitraPrint("GEAR-LINK camera-matrix=%u %08x %08x %08x %08x\n",
                    row, matrix[0], matrix[1], matrix[2], matrix[3]);
            }
        }
    }
    ...
    for (u32 stage = 0; stage < 6; ++stage) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            CitraPrint("GEAR-LINK pass=%u index=%u model=%08x raw=%08x override=%u\n",
                stage, i, (u32)model, model[5], ((const u8*)model)[0xac]);
            for (u32 row = 0; row < 3; ++row) {
                const u32* matrix = model + 0x7c/4 + row*4;
...
void GearPreview_Probe(GlobalContext* globalCtx) {
    ...
    const Player* player = globalCtx && gSaveContext.gameMode == 0 ?
        globalCtx->mainCamera.player : 0;
    ...
    const u8 active = globalCtx && gSaveContext.gameMode == 0 &&
        InputRemap_IsGearPageActive() && !UnifiedMenu_IsOpening() &&
    ...
    if (!active) { previousActive = 0; return; }
    /* Advance once per active menu update, not once per render pass. */
    sIdlePhase += 384;
    if (!player) return;
    if (sPreview && !sRetired) sDrawReady = 1;
    ...
    CitraPrint("GEAR-LINK age=%d equip=%04x sword=%u shield=%u tunic=%u boots=%u\n",
        (int)gSaveContext.linkAge, equipment, equipment & 15,
        (equipment >> 4) & 15, (equipment >> 8) & 15, equipment >> 12);
    const SkeletonAnimationModel* model = player->skelAnime.unk_28;
    if (!model || !model->vtbl) return;
    const u32* vtable = (const u32*)model->vtbl;
    CitraPrint("GEAR-LINK model=%08x update=%08x destroy=%08x raw=%08x\n",
        (u32)model, vtable[2], vtable[1], (u32)model->unk_draw_struct_14);
    ...
    CitraPrint("GEAR-LINK actor-tunic=%u shield=%u boots=%u resource=%08x\n",
        actor[0x1a4], actor[0x1a6], actor[0x1a7],
        *(const u32*)(actor + 0x24dc));
    if (!sPreview) sScene = globalCtx->sceneNum;
