...
static inline unsigned OcarinaPresentation_Update(unsigned previous,
        unsigned allowed, unsigned instrumentOpen, unsigned messageActive) {
    if (!allowed) return 0;
    if (instrumentOpen) return 1;
