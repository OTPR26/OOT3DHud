...
int main(void) {
    ...
    for (unsigned slot = 0; slot < 6; ++slot) {
        ...
        PrefEncode(a, slot, 123, 1, v);
        PrefEncode(b, slot, 123, 2, v);
        assert(PrefValid(a, slot, 123));
        assert(PrefNewest(a, b, slot, 123) == 1);
        assert(!PrefValid(a, (slot + 1) % 6, 123));
        assert(!PrefValid(a, slot, 124));
        assert(!memcmp(a + 6, v, 6) && !memcmp(a + 20, v + 6, 3));
        for (unsigned byte = 0; byte < PREF_SIZE; ++byte) {
            b[byte] ^= 1;
            assert(PrefNewest(a, b, slot, 123) == 0);
            b[byte] ^= 1;
        }
        for (unsigned size = 0; size < PREF_SIZE; ++size) {
            memset(b, 0, PREF_SIZE);
            memcpy(b, a, size);
            assert(!PrefValid(b, slot, 123));
        }
        PrefEncode(a, slot, 123, 0xffffffffu, v);
        PrefEncode(b, slot, 123, 0, v);
        assert(PrefNewest(a, b, slot, 123) == 1);
    }
