...
#if defined(Version_USA) || (defined(Version_JP) && JP_SINGLE_SCREEN_PROBE) || (defined(Version_TWN) && TWN_SINGLE_SCREEN_PROBE) || (defined(Version_KOR) && KOR_SINGLE_SCREEN_PROBE)

#if defined(Version_KOR)
#define SET_VIEWPORT_ADDR 0x0031DBE4
#define FINISH_TOP_PASS_ADDR 0x0010C66C
#define DRAW_SECONDARY_UI_ADDR 0x001088DC
#define DRAW_SECONDARY_WIDGETS_ADDR 0x00108F5C
#define GPU_COMMAND_CURRENT_ADDR 0x0055ACD0
#define GPU_COMMAND_END_ADDR 0x0055ACD4
#define BOARD_VTABLE_ADDR 0x004F431C
#define DRAW_FILE_LOGO_ADDR 0x0010C67C
#define UPDATE_FILE_BOARD_ADDR 0x00317030
#define DRAW_TITLE_ADDR 0x001050AC
...
void SingleScreen_UpdateFileBoard(void* board) {
    ...
    for (u32 q = 0; q < 6; ++q) {
        ...
        for (u32 v = 0; v < 4; ++v) {
#if defined(Version_JP) || defined(Version_TWN) || defined(Version_KOR)
...
void SingleScreen_AfterTopPass(void* pass) {
    ...
    if (InputRemap_IsGameOverUiOpen()) {
        ...
        void* const deathMenu = *(void* volatile*)0x005CCDA4;
        if (deathMenu != 0 && gGlobalContext != 0) {
            ((void (*)(void*, GlobalContext*))0x00106260)(
