...
void Draw_DrawPixelTop(u32 posX, u32 posY, u32 color)
{
    if(posX >= SCREEN_TOP_WIDTH || posY >= SCREEN_TOP_HEIGHT) return;

    const u32 screenPos = posX * SCREEN_TOP_HEIGHT + (SCREEN_TOP_HEIGHT - posY - 1);
    for(u32 i = 2; i < 6; i++)
    {
        volatile u8 *const fb = (volatile u8 *const)FRAMEBUFFER[i];
        fb[screenPos * 3] = color & 0xFF;
        fb[screenPos * 3 + 1] = (color >> 8) & 0xFF;
        fb[screenPos * 3 + 2] = (color >> 16) & 0xFF;
    }
}
...
void Draw_DrawRectTop(u32 posX, u32 posY, u32 width, u32 height, u32 color)
{
    for(u32 x = 0; x < width; x++)
        for(u32 y = 0; y < height; y++)
            Draw_DrawPixelTop(posX + x, posY + y, color);
}

void Draw_DrawRectOutlineTop(u32 posX, u32 posY, u32 width, u32 height, u32 color)
{
    if(width == 0 || height == 0) return;
    Draw_DrawRectTop(posX, posY, width, 1, color);
    Draw_DrawRectTop(posX, posY + height - 1, width, 1, color);
    Draw_DrawRectTop(posX, posY, 1, height, color);
    Draw_DrawRectTop(posX + width - 1, posY, 1, height, color);
}

void Draw_DrawSpritePartTop(u32 posX, u32 posY, u32 drawWidth, u32 height, u32 sourceWidth,
                           const u8* rgba2222)
{
    if(!rgba2222) return;
    for(u32 y = 0; y < height; y++)
    {
        ...
        {
            const u8 texel = rgba2222[y * sourceWidth + x];
            const u32 alpha = (texel >> 6) & 0x3;
            if(alpha < 2 || posX + x >= SCREEN_TOP_WIDTH || posY + y >= SCREEN_TOP_HEIGHT) continue;

            const u32 red = ((texel >> 4) & 0x3) * 85;
            const u32 green = ((texel >> 2) & 0x3) * 85;
            const u32 blue = (texel & 0x3) * 85;
            Draw_DrawPixelTop(posX + x, posY + y, RGB8(red, green, blue));
        }
    }
}

void Draw_DrawSpriteTop(u32 posX, u32 posY, u32 width, u32 height, const u8* rgba2222)
{
    Draw_DrawSpritePartTop(posX, posY, width, height, width, rgba2222);
}

void Draw_DrawOverlaidCharacterTop(u32 posX, u32 posY, u32 color, char character)
{
    for(s32 y = 0; y < 10; y++)
    {
        const char charPos = ascii_font[character * 10 + y];
        for(s32 x = 6; x >= 1; x--)
            if((charPos >> x) & 1)
                Draw_DrawPixelTop(posX + (5 - x), posY + y, color);
    }
}
