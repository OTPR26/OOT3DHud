...
static int Open(Handle* file,u32 flags) {
    ...
    if (!sFs) {
        if (srvGetServiceHandle(&sFs,"fs:USER")<0) { sFs=0; return 0; }
        c[0]=IPC_MakeHeader(0x801,0,2); c[1]=IPC_Desc_CurProcessId(); c[2]=0;
        if (Request(sFs)<0) { svcCloseHandle(sFs); sFs=0; return 0; }
    }
    char path[34];
    PrefPath(path,sSlot);
    c[0]=IPC_MakeHeader(0x803,8,4);
    c[1]=0; c[2]=9; c[3]=1; c[4]=1; c[5]=3; c[6]=sizeof(path);
    c[7]=flags; c[8]=0;
    c[9]=IPC_Desc_StaticBuffer(1,2); c[10]=(u32)"";
    c[11]=IPC_Desc_StaticBuffer(sizeof(path),0); c[12]=(u32)path;
    Result result=Request(sFs);
    if (result<0) { CitraPrint("PREF open slot=%d flags=%u result=%08x\n",sSlot,flags,(u32)result); return 0; }
    *file=c[3]; return 1;
}
static void Close(Handle file) {
    getThreadCommandBuffer()[0]=IPC_MakeHeader(0x808,0,0);
    Request(file); svcCloseHandle(file);
}
static int Transfer(Handle file,u8* bytes,u32 offset,int write) {
    u32* c=getThreadCommandBuffer();
    c[0]=IPC_MakeHeader(write ? 0x803 : 0x802,write ? 4 : 3,2);
    c[1]=offset; c[2]=0; c[3]=PREF_SIZE;
    unsigned desc=write ? 5 : 4;
    if (write) c[4]=1; /* flush this record before reporting success */
...
static int ReadRecords(u8 records[2][PREF_SIZE]) {
    ...
    for (u32 i=0;i<2;++i)
        if (!Transfer(file,records[i],i*PREF_SIZE,0)) memset(records[i],0,PREF_SIZE);
    Close(file);
...
static void Apply(const u8* v) {
    ...
    save[0x2d]=v[0]; save[0x13d8]=v[1]; save[0xf]=v[2];
    static const u8 sizes[]={0,75,100,125};
    NativeHud_SetScalePercent(sizes[v[3]]);
    Camera_SetDistanceOption(v[4]); Camera_SetControlMode(v[5]);
    for (unsigned i=0;i<3;++i) MovementOptions_Set(i,v[6+i]);
}
void MenuPreferences_Update(GlobalContext* context) {
    if (!context || gSaveContext.gameMode!=0) { sSlot=-1; return; }
    ...
    if (sSlot==slot && sIdentity==identity) return;
    sSlot=slot; sIdentity=identity;
    /* Missing/invalid preferences must never inherit another slot's HUD. */
    NativeHud_SetScalePercent(100); Camera_SetDistanceOption(2); Camera_SetControlMode(0);
    for (unsigned i=0;i<3;++i) MovementOptions_Set(i,0);
    u8 records[2][PREF_SIZE]; int latest=ReadRecords(records);
    if (latest>=0) {
        u8 values[PREF_VALUES];
        memcpy(values,records[latest]+6,6); memcpy(values+6,records[latest]+20,3);
        Apply(values);
    }
    CitraPrint("PREF load slot=%d identity=%08x generation=%d\n",sSlot,sIdentity,latest);
}
int MenuPreferences_Save(const u8 values[9]) {
    if (sSlot<0 || sSlot>5 || !PrefValuesValid(values)) return 0;
    ...
    Handle file;
    if (!Open(&file,7)) return 0;
    int success=Transfer(file,encoded,target*PREF_SIZE,1);
    Close(file);
    if (!success || !Open(&file,1)) return 0;
    success=Transfer(file,verify,target*PREF_SIZE,0) && PrefEqual(encoded,verify,PREF_SIZE);
    Close(file);
    CitraPrint("PREF saved slot=%d generation=%u verified=%d\n",sSlot,generation,success);
    return success;
}
#else
void MenuPreferences_Update(GlobalContext* context) { (void)context; }
int MenuPreferences_Save(const u8 values[9]) { (void)values; return 0; }
#endif
