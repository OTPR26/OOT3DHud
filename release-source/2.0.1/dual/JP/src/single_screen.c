...
void SingleScreen_BeforeFileScene(void) {
    /* Keep the native upper-screen Link model viewport. */
}
...
static u8 IsGameplayDialogueActive(void) {
    return 0; /* Preserve Dual Screen gameplay dialogue. */
}
        ...
        if (text && text[0x40/4] == 2) {
            ...
            if (foreground) {
                ...
                for (u32 n = 0; n < 9; ++n) {
                    ...
                    node[0x40/4] = centerY + (node[0x40/4] - centerY) * 0.8f;
...
void SingleScreen_AfterTopPass(void* pass) {
    ...
    const u8 showFileSelect = 0; /* Native file UI stays below. */
