...
static u8 NativeHud_IsInstrumentActive(void) {
#if defined(Version_EUR) || defined(Version_USA) || defined(Version_JP)
    // EUR 0x425930, USA 0x42590C and JP 0x4258E4 read manager 0x5093E4 + 0x14
    // and returns immediately when zero. Read only; never alter menu state.
    return *(volatile u32*)0x005093F8 != 0;
#else
    return 0;
#endif
}
...
static void NativeHud_RebuildPositions(void) {
    ...
    if (sNativeHudOcarinaLayout) {
        // Borrow the I-ammo quad only while instrument mode is active.
        float* p = gNativeHudPositions + 12 * 4 * 3;
        p[0] = p[6] = 400.0f - 26.0f * scale;
        p[3] = p[9] = 400.0f - 10.0f * scale;
        p[1] = p[4] = 45.0f * scale;
        p[7] = p[10] = 29.0f * scale;
    }
    ...
    NativeHud_ScaleQuadRange(8, 11, 0.0f, 0.0f, scale);
    if (!sNativeHudOcarinaLayout)
        NativeHud_ScaleQuad(12, 0.0f, 0.0f, scale);
    NativeHud_ScaleQuadRange(13, 23, 0.0f, 0.0f, scale);
...
static void NativeHud_UpdateMappedItems(u8 visible) {
    ...
    if (sNativeHudOcarinaLayout) {
        // Same blank-circle cell as the three permanent face-button bases.
        for (u32 i = 0; i < 8; i++)
            gNativeHudUVs[12 * 8 + i] = gNativeHudUVs[1 * 8 + i];
    }
...
void NativeHud_Update(GlobalContext* globalCtx) {
    const u8 instrument = globalCtx != 0 && IsInGameOrBossChallenge() &&
                          NativeHud_IsInstrumentActive();
    if (instrument != sNativeHudOcarinaLayout) {
        sNativeHudOcarinaLayout = instrument;
        NativeHud_RebuildPositions();
    }
...
u8 NativeHud_GetScalePercent(void) { return sNativeHudScalePercent; }
void NativeHud_SetScalePercent(u8 percent) {
    if (percent!=0 && percent!=75 && percent!=100 && percent!=125) return;
    sNativeHudScalePercent=percent; NativeHud_RebuildPositions();
}
